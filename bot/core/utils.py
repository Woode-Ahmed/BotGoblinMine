...
def load_codes_from_files() -> dict:
    with Path("codes.json").open("r", encoding="utf-8") as file:
...
def num_prettier(num: int) -> str:
    ...
    if number >= (comparer := 1e12):
        ...
        prettier_num = f"{number:.1f}"
    return f"-{prettier_num}" if num < 0 else prettier_num


async def is_current_hour_in_range(start_hour: int, end_hour: int, logger: Logger) -> bool:
    ...
    if result:
        ...
        hours_to_sleep = end_hour - current_hour if current_hour < end_hour else 24 - current_hour + end_hour

        logger.info(f"🌙 The bot is sleeping 🛌 in night mode. " f"Sleep time {hours_to_sleep} hours 🕖. ")
        await asyncio.sleep(
            hours_to_sleep * 3600 + random.randint(config.RANDOM_SLEEP_TIME * 5, config.RANDOM_SLEEP_TIME * 20)
        )
