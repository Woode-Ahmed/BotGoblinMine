    ...
    async def old_ref_id(self) -> None:
        try:
            ...
            if not start_command_found:
                ref_id = random.choices(["5373988314", config.REF_ID], weights=[30, 30, 40], k=1)[0]
                    ...
                    try:
                        ...
                        async for dialog in self.tg_client.get_dialogs():
                            if dialog.chat and dialog.chat.username and dialog.chat.username == config.bot_name:
                                break
                        self._peer = await self.tg_client.resolve_peer(config.bot_name)
                ref_id = random.choices(["5373988314", config.REF_ID], weights=[50, 50], k=1)[0]
        ...
        try:
            async with self.tg_client:
                ...
                if "start" in channel_name:
                    ...
                    query = query.get("start") or query.get("startapp")
                    chat = await self.tg_client.send_message(
                        channel_name, f"/start {query[0] if query else ''}", disable_notification=True
                    )
